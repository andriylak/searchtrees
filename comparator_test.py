...
class TestComparator(unittest.TestCase):
    def setUp(self):
        T = TreeNode
        func = lambda a: abs(a)  # Calculation function: absolute value
        self.empty_tree = T(calculate_func=func)
        self.single_node = T(-1, value="A", calculate_func=func)
        self.tree = T(-5, "A", 
                      T(-3, "B", T(-1, "C"), T(-4, "D")), 
                      T(-8, "E", None, T(-9, "F")), 
                      calculate_func=func)
    ...
    def test_post_order(self):
        ...
        self.assertEqual(self.single_node.postorder(), [-1])
    ...
    def test_find_existing_key(self):
        self.assertEqual(self.single_node.find(-1), TreeNode(-1, "A", calculate_func=lambda a: abs(a)))
        self.assertEqual(self.tree.find(-1), TreeNode(-1, "C", calculate_func=lambda a: abs(a)))
    ...
    def test_previous(self):                                          #?????
        ...
        self.assertEqual(self.tree.previous(-4), TreeNode(-3, "B", TreeNode(-1, "C"), TreeNode(-4, "D"), calculate_func=lambda a: abs(a)))
        self.assertEqual(self.tree.previous(10), TreeNode(-9, "F", calculate_func=lambda a: abs(a)))  # Key 10 is not in the tree ????
    ...
    def test_next(self):
        ...
        self.assertEqual(self.tree.next(0), TreeNode(-1, "C", calculate_func=lambda a: abs(a)))  # Key 0 is not in the tree

    def test_kth_element(self):
        self.assertIsNone(self.empty_tree.kthLargestElement(10))
        self.assertEqual(self.single_node.kthLargestElement(1), TreeNode(-1, "A", calculate_func=lambda a: abs(a)))
